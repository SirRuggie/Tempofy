...
import { SoundModule } from './src/utils/sounds'
...
export default function App() {
  ...
  const initializeApp = async () => {
    try {
      await SoundModule.initialize()
      const savedTasks = await StorageModule.getTasks()
      const savedTempo = await StorageModule.getCurrentTempo()
      
      setTasks(savedTasks)
      setCurrentTempo(savedTempo)
    } catch (error) {
  ...
  const handleAddNote = async (noteData: {
    ...
    await SoundModule.playNotification()
  }
  ...
  const handleCompleteTask = async (taskId: string) => {
    ...
    setTasks(updatedTasks)
    await StorageModule.saveTasks(updatedTasks)
    await SoundModule.playCompletionSound(task.tempo_required)
  }

  const handleTempoChange = async (tempo: Tempo) => {
    setCurrentTempo(tempo)
    await StorageModule.saveCurrentTempo(tempo)
    await SoundModule.playNotification()
  }
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Setting up your symphony...</Text>
      </View>
    )
  }
